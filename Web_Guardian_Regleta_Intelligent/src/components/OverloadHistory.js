...
const OverloadHistory = () => {
  ...
  useEffect(() => {
    const fetchOverloads = async () => {
      try {
        const response = await axios.get('http://localhost:4000/energy/history');
...
const handleDeleteHistory = async () => {
  ...
  try {
    await axios.delete('http://localhost:4000/energy/history');
    setOverloads([]);
...
const handleDeleteSingle = async (id) => {
  ...
  try {
    await axios.delete(`http://localhost:4000/energy/history/${id}`);
